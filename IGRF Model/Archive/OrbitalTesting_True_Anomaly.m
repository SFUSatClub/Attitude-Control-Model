...
Argument_of_Latitude = 0 %radians
Argument_of_Perigee = 0; %radians only for eccentricity != 0
Time_Interval = 10;
RAAN = 0;

Result_Vector = [0, 0];
for Argument_of_Latitude = 0:0.01:2*pi
    ...
    %   Solve for New True Anomaly when ellipse is as a circle
    fun = @(variable1) variable1*(1-eccentricity*sin(variable1))-(2*New_Area/(semimajor_axis*semiminor_axis));
    New_Eccentricity_True_Anomaly = fzero(fun, Current_Eccentricity_True_Anomaly);
    if eccentricity == 0 || New_Eccentricity_True_Anomaly == pi%  Skip secondary iterative solution if eccentricity is already a circle
        New_True_Anomaly = New_Eccentricity_True_Anomaly;
    else
        %   Solve for True Anomaly after converting the circle back into
        %   ellipse. Has a singularity at variable2 = pi
        TestMethod = 2*atan(tan(New_Eccentricity_True_Anomaly/2)*sqrt((1+eccentricity)/(1-eccentricity)));
        fun2 = @(variable2) 2*atan(sqrt((1-eccentricity)/(1+eccentricity))*tan(variable2/2))-New_Eccentricity_True_Anomaly; 
        New_True_Anomaly = fzero(fun2, Current_True_Anomaly);
    end
    % Establish New_Argument_of_Latitude (Easier for converting to ECEF?)
    ...
    Error = New_Area - 0.5*semimajor_axis*semiminor_axis*(New_Eccentricity_True_Anomaly-eccentricity*sin(New_Eccentricity_True_Anomaly));
    Result_Vector = [Result_Vector; Argument_of_Latitude, Error];
end
